...
2. **[ ] Gate 2: Review Context**: Query the local knowledge graph first — `npx --no-install docuvia query "<concept_or_file>" --format=prompt` — then use `search` and `read` to understand the existing context and terminology.
