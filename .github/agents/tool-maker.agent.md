...
1. **[ ] Gate 1: Understand the Bottleneck**: Read the context provided regarding what operation is failing or is too repetitive. If it concerns an existing module, query the local knowledge graph first — `npx --no-install docuvia query "<concept_or_file>" --format=prompt`.
