...
| `lib/semantic-decision`             | `@workspace/semantic-decision` | Technology Provider | PLAT-011 isolated local semantic decision/model runtime; only model-agnostic contracts may cross the package boundary. |
| `lib/ast-core`                      | `@workspace/ast-core`    | Technology Provider | AST microkernel: tree-sitter bridge, traversal, edge computation, compression, language detection.                                                                                                              |
...
| `artifacts/cli`                     | `docuvia`                | Presentation Layer  | The `docuvia` CLI — argument parsing, per-platform `init` installers, output formatting. Published to npm as [`docuvia`](https://www.npmjs.com/package/docuvia); version is release-managed (semantic-release). |
...
- the dependency is one of `impact`'s blind spots: a plugin path built from a runtime variable, `import()` with a computed specifier, or `child_process` spawning another project file. The call-site fallback does **not** recover these (all three measure 0.000 precision/recall in the eval corpus) — none of them names the target at a call site, so there is no row to reverse-look-up. Use Grep for these. What the fallback _does_ recover is a **bare-identifier** call the static resolver could not link; those appear as lower-confidence dependents labeled `lsp-fallback` in the Source column (`edgeSource` in JSON) — read them as "a call to this name exists here" and cross-check before relying on them. Method calls written `obj.method()` are **not** recovered today: the call site is stored as `obj.method` while the lookup matches `method`. See [`impact`'s "What counts as a dependency edge"](docs/gitbook/user-guide/cli/impact.md#what-counts-as-a-dependency-edge).
