...
- **After making a code change that reflects a real architectural decision, rule, or notable rationale**: write it via `npx --no-install docuvia analyze <file> --agent-authored` (through `Bash`) — pipe a JSON payload on stdin (default) or pass `--decisions-file=<path>`, shape `{"decisions":[{"title":string,"content":string,"nodeType":"change"|"rule"|"decision"|"context","confidence":number}]}`. Put `--agent-authored` **after** the positional `<file>`, not before — a flag preceding the path silently swallows it as the flag's own value. It lands in the graph immediately, in one pass: no LLM call, no scheduling, nothing further to run. Do **not** add `--stage` as a matter of course — that defers the write until a later commit touching that same file, so a decision about a file you don't revisit waits indefinitely and silently.
