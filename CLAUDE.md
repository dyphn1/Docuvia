...
- Fall back to `Grep`/`Glob`/`Read` only per the exceptions listed in AGENTS.md (empty/unprocessed graph result, need for exact source text, or a dynamic dependency `impact` can't see).
