...
export async function performVectorSearch(
  ...
  if (projectId) {
    ...
    conditions.push(
      sql`${l3NodesTable.l2NodeId} IN (SELECT id FROM l2_nodes WHERE project_id = ${projectId})`
    );
  }
  if (!includePending) {
  ...
  const query = db
    ...
    .where(whereClause)
    .orderBy(
      sql`(1 - (embedding::vector <=> ${vectorStr}::vector)) * EXP(-0.05 * EXTRACT(EPOCH FROM (NOW() - COALESCE(last_verified_at, created_at)))/86400) DESC`
    )
    .limit(limit);
