...
import { projectsTable, l1TagsTable, l2NodesTable, l3NodesTable, reviewTasksTable, activityLogTable, projectsTable as proj } from "@workspace/db";
...
router.get("/dashboard", async (req, res) => {
  ...
  const [pendingCount] = await db.select({ count: count() }).from(reviewTasksTable).where(eq(reviewTasksTable.status, "pending"));
