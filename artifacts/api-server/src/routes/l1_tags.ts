...
router.get("/l1-tags", async (req, res) => {
  const tags = await db.select().from(l1TagsTable).orderBy(sql`${l1TagsTable.usageCount} desc`);
...
router.post("/l1-tags", async (req, res) => {
  ...
  const [tag] = await db.insert(l1TagsTable).values({
    name: body.name,
...
router.patch("/l1-tags/:id", async (req, res) => {
  ...
  res.json({ ...tag, createdAt: tag.createdAt.toISOString() });
});
