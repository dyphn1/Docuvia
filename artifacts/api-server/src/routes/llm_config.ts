...
router.get("/projects/:id/llm-config", async (req, res) => {
  ...
  let [cfg] = await db.select().from(llmConfigsTable).where(eq(llmConfigsTable.projectId, projectId));
  if (!cfg) {
    [cfg] = await db.insert(llmConfigsTable).values({
      projectId,
      provider: "openai",
      model: "gpt-5.2",
      isDefault: false,
...
router.patch("/projects/:id/llm-config", async (req, res) => {
  ...
  if (!cfg) {
    ...
    [cfg] = await db.update(llmConfigsTable).set({
      ...(body.model && { model: body.model }),
      ...(body.provider && { provider: body.provider }),
      updatedAt: new Date(),
    }).where(eq(llmConfigsTable.id, cfg.id)).returning();
  }
  res.json({ ...cfg, createdAt: cfg.createdAt.toISOString() });
});
