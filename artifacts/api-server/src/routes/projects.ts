...
import { projectsTable, l2NodesTable, l3NodesTable, commitsTable, activityLogTable } from "@workspace/db";
...
router.get("/projects", async (req, res) => {
  ...
  const result = await Promise.all(
    projects.map(async (p) => {
      const [l2CountRow] = await db.select({ count: count() }).from(l2NodesTable).where(eq(l2NodesTable.projectId, p.id));
      ...
      const [commitCountRow] = await db.select({ count: count() }).from(commitsTable).where(eq(commitsTable.projectId, p.id));
      return {
        ...p,
...
router.post("/projects", async (req, res) => {
  ...
  res.status(201).json({ ...project, l2Count: 0, l3Count: 0, commitCount: 0, createdAt: project.createdAt.toISOString(), updatedAt: project.updatedAt.toISOString() });
});
...
router.get("/projects/:id", async (req, res) => {
  ...
  const [l2CountRow] = await db.select({ count: count() }).from(l2NodesTable).where(eq(l2NodesTable.projectId, id));
  const l2Ids = await db.select({ id: l2NodesTable.id }).from(l2NodesTable).where(eq(l2NodesTable.projectId, id));
  let l3Count = 0;
  for (const { nodeId } of l2Ids as any[]) {
    const lid = nodeId ?? (l2Ids[0] as any).id;
    const [row] = await db.select({ count: count() }).from(l3NodesTable).where(eq(l3NodesTable.l2NodeId, lid));
    l3Count += row.count;
  }
  const [commitCountRow] = await db.select({ count: count() }).from(commitsTable).where(eq(commitsTable.projectId, id));
  res.json({ ...project, l2Count: l2CountRow.count, l3Count, commitCount: commitCountRow.count, createdAt: project.createdAt.toISOString(), updatedAt: project.updatedAt.toISOString() });
});

router.patch("/projects/:id", async (req, res) => {
  const { id } = UpdateProjectParams.parse({ id: Number(req.params.id) });
  const body = UpdateProjectBody.parse(req.body);
  const [project] = await db.update(projectsTable).set({ ...body, updatedAt: new Date() }).where(eq(projectsTable.id, id)).returning();
  if (!project) return res.status(404).json({ error: "Not found" });
  res.json({ ...project, l2Count: 0, l3Count: 0, commitCount: 0, createdAt: project.createdAt.toISOString(), updatedAt: project.updatedAt.toISOString() });
});
...
router.get("/projects/:id/graph", async (req, res) => {
  ...
  const l3Nodes = l2Nodes.length
    ? await db.select().from(l3NodesTable).where(
        sql`${l3NodesTable.l2NodeId} IN (${sql.join(l2Nodes.map(n => sql`${n.id}`), sql`, `)})`
      )
    : [];
  res.json({
    projectId: id,
    l1Tags: [],
    l2Nodes: l2Nodes.map(n => ({ ...n, l3Count: 0, l1TagIds: [], createdAt: n.createdAt.toISOString() })),
...
router.get("/projects/:id/commits", async (req, res) => {
  ...
  const commits = await db.select().from(commitsTable).where(eq(commitsTable.projectId, id)).orderBy(sql`${commitsTable.createdAt} desc`);
...
router.get("/projects/:id/l2-nodes", async (req, res) => {
  ...
  const result = await Promise.all(
    nodes.map(async (n) => {
      const [row] = await db.select({ count: count() }).from(l3NodesTable).where(eq(l3NodesTable.l2NodeId, n.id));
