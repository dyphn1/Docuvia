...
export class L3NodesService {
  ...
  async updateNode(id: number, body: z.infer<typeof UpdateL3NodeBody>) {
    const [existing] = await db.select().from(l3NodesTable).where(eq(l3NodesTable.id, id));
    let embedding: number[] | null = null;
    if (existing && (body.title !== undefined || body.content !== undefined)) {
      const [l2] = await db
        .select()
        .from(l2NodesTable)
        .where(eq(l2NodesTable.id, existing.l2NodeId));
      const title = body.title ?? existing.title;
      const content = body.content ?? existing.content ?? "";
      embedding = await generateEmbedding(l2?.projectId, `${title} ${content}`.trim());
    }

    const [node] = await db
      .update(l3NodesTable)
      .set({ ...body, ...(embedding ? { embedding } : {}) } as any)
