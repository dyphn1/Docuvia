...
describe("MCP API", () => {
  ...
  it("GET /api/mcp/list_projects", () =>
    withRollback(async () => {
      const res = await request(app).get("/api/mcp/list_projects").set(authHeaders);
      expect(res.status).toBe(200);
    }));

  it("GET /api/mcp/search_knowledge", () =>
    withRollback(async () => {
      const res = await request(app).get("/api/mcp/search_knowledge?query=test").set(authHeaders);
      expect(res.status).toBe(200);
    }));

  it("GET /api/mcp/get_dependencies", () =>
    withRollback(async () => {
      const res = await request(app)
        .get("/api/mcp/get_dependencies?module=test&project_id=1")
        .set(authHeaders);
      expect(res.status).toBe(200);
    }));

  it("GET /api/mcp/impact_analysis", () =>
    withRollback(async () => {
      const res = await request(app)
        .get("/api/mcp/impact_analysis?module=test&project_id=1")
        .set(authHeaders);
      expect(res.status).toBe(200);
    }));

  it("GET /api/mcp/get_decision_record", () =>
    withRollback(async () => {
      const res = await request(app)
        .get("/api/mcp/get_decision_record?commit_hash=abc")
        .set(authHeaders);
      expect(res.status).toBe(200);
    }));

  it("POST /api/mcp/query", () =>
    withRollback(async () => {
      const res = await request(app).post("/api/mcp/query").set(authHeaders).send({ q: "test" });
      expect(res.status).toBe(200);
    }));

  it("GET /api/mcp/tools", () =>
    withRollback(async () => {
