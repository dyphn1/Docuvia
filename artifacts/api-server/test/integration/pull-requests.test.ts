...
import { ProjectFactory, CommitFactory, L2NodeFactory, L3NodeFactory } from "../support/factories.js";
...
describe("Pull Requests API", () => {
  it("GET /api/projects/:id/pull-requests returns PRs", () => withRollback(async () => {
  ...
  it("GET /api/projects/:id/pull-requests/:prNumber returns details", () => withRollback(async () => {
    const project = await ProjectFactory.create();
    const pr = (await db.insert(pullRequestsTable).values({
      projectId: project.id,
      githubPrNumber: 102,
      title: "Feature X",
      status: "open",
  ...
  it("POST /api/projects/:id/pull-requests/:prNumber/analyze triggers analysis", () => withRollback(async () => {
    const project = await ProjectFactory.create();
    await db.insert(pullRequestsTable).values({
      projectId: project.id,
      githubPrNumber: 103,
