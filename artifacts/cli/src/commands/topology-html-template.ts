...
export function renderTopologyHtml(graph: TopologyGraph): string {
  const dataJson = JSON.stringify(graph).replace(/</g, String.fromCharCode(92) + "u003c");
...
function renderHead(graph: TopologyGraph): string {
  ...
  lines.push('<title>Docuvia Topology - ' + escapeHtml(graph.workspaceRoot) + '</title>');
  lines.push('<style>');
  lines.push('* { box-sizing: border-box; margin: 0; padding: 0; }');
  lines.push('body { background: #0f1420; color: #dbe2ef; font-family: sans-serif; display: flex; height: 100vh; overflow: hidden; }');
  lines.push('#graph { flex: 1; cursor: grab; }');
  lines.push('#graph.dragging { cursor: grabbing; }');
  lines.push('#sidebar { width: 360px; background: #161c2c; border-right: 1px solid #263048; display: flex; flex-direction: column; overflow: hidden; }');
  lines.push('.panel { padding: 12px; border-bottom: 1px solid #263048; }');
  lines.push('.panel h3 { font-size: 12px; color: #8b98b8; margin-bottom: 8px; text-transform: uppercase; }');
  lines.push('#search { width: 100%; background: #0f1420; border: 1px solid #33405f; color: #dbe2ef; padding: 7px 10px; border-radius: 6px; font-size: 13px; outline: none; }');
  lines.push('#search-results { max-height: 130px; overflow-y: auto; margin-top: 6px; }');
  lines.push('.search-item, .neighbor-item { padding: 4px 6px; cursor: pointer; border-radius: 4px; font-size: 12px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }');
  lines.push('.search-item:hover, .neighbor-item:hover { background: #263048; }');
  lines.push('#info { font-size: 13px; line-height: 1.65; min-height: 120px; }');
  lines.push('#info .muted { color: #5d6a8a; font-style: italic; }');
  lines.push('#info b { color: #f1f4fb; }');
  lines.push('.kind-badge { display: inline-block; padding: 1px 7px; border-radius: 9px; font-size: 11px; margin-left: 6px; }');
  lines.push('.kind-file { background: #2a4d7a; } .kind-symbol { background: #3a6b4f; } .kind-decision { background: #8a6d1f; }');
  lines.push('#neighbors { max-height: 150px; overflow-y: auto; margin-top: 4px; }');
  lines.push('#legend { flex: 1; overflow-y: auto; padding: 12px; }');
  lines.push('.legend-item { display: flex; align-items: center; gap: 8px; padding: 4px 2px; cursor: pointer; border-radius: 4px; font-size: 12px; }');
  lines.push('.legend-item:hover { background: #263048; }');
  lines.push('.legend-item.off { opacity: 0.3; }');
  lines.push('.legend-dot { width: 11px; height: 11px; border-radius: 50%; flex-shrink: 0; }');
  lines.push('.legend-label { flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }');
  lines.push('.legend-count { color: #5d6a8a; font-size: 11px; }');
  lines.push('#stats { padding: 10px 12px; border-top: 1px solid #263048; font-size: 11px; color: #5d6a8a; }');
  lines.push('#controls { position: absolute; top: 16px; right: 16px; display: flex; gap: 8px; z-index: 10; }');
  lines.push('.btn { background: #161c2c; color: #dbe2ef; border: 1px solid #33405f; padding: 6px 10px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: bold; outline: none; }');
...
function renderBody(): string {
  ...
  lines.push('<button class="btn" id="btn-zoom-out" title="Zoom Out">-</button>');
  lines.push('<button class="btn" id="btn-fit" title="Fit View">Fit View</button>');
  lines.push('</div>');
  lines.push('<div id="sidebar">');
  lines.push('<div class="panel">');
  lines.push('<input id="search" type="text" placeholder="Search nodes..." autocomplete="off">');
  ...
  lines.push('</div>');
  lines.push('<div id="legend"><h3>Groups</h3><div id="legend-items"></div></div>');
...
function scriptSetup(dataJson: string): string {
  ...
  lines.push('var PALETTE = ["#4E79A7","#F28E2B","#E15759","#76B7B2","#59A14F","#EDC948","#B07AA1","#FF9DA7","#9C755F","#BAB0AC","#5B8DEF","#8CD17D"];');
  ...
  lines.push('var nodes = GRAPH.nodes.map(function (n) { return Object.assign({ x: 0, y: 0, vx: 0, vy: 0 }, n); });');
  lines.push('var nodeById = {};');
  lines.push('nodes.forEach(function (n) { nodeById[n.id] = n; });');
  lines.push('var links = GRAPH.links.filter(function (l) { return nodeById[l.source] && nodeById[l.target]; });');
  ...
  lines.push('nodes.forEach(function (n) { if (n.degree > maxDeg) maxDeg = n.degree; });');
  ...
  lines.push('  groupCenters[g.id] = { x: R * Math.cos(a), y: R * Math.sin(a) };');
  lines.push('});');
...
function scriptSimulation(): string {
  ...
  lines.push('    var rest = l.linkType === "contains" || l.linkType === "decision" ? 60 : 140;');
  ...
  lines.push('  scale = Math.min(w / (maxX - minX + 200), h / (maxY - minY + 200), 2);');
  ...
  lines.push('        if (!(l.source in seen)) { seen[l.source] = depth + 1; next.push(l.source); }');
  lines.push('      });');
  lines.push('    });');
  ...
  lines.push('  return n.kind === "decision" ? 6 : 6 + 14 * Math.sqrt(n.degree / maxDeg);');
  lines.push('}');
  ...
  lines.push('  pts = pts.slice().sort(function (a, b) { return a.x - b.x || a.y - b.y; });');
  lines.push('  var cross = function (o, a, b) { return (a.x - o.x) * (b.y - o.y) - (a.y - o.y) * (b.x - o.x); };');
  lines.push('  var lower = [], upper = [], k;');
  lines.push('  for (k = 0; k < pts.length; k++) {');
  lines.push('    while (lower.length >= 2 && cross(lower[lower.length - 2], lower[lower.length - 1], pts[k]) <= 0) lower.pop();');
  lines.push('    lower.push(pts[k]);');
  lines.push('  }');
  lines.push('  for (k = pts.length - 1; k >= 0; k--) {');
  lines.push('    while (upper.length >= 2 && cross(upper[upper.length - 2], upper[upper.length - 1], pts[k]) <= 0) upper.pop();');
...
function scriptDraw(): string {
  ...
  lines.push('  ctx.setTransform(devicePixelRatio, 0, 0, devicePixelRatio, 0, 0);');
  lines.push('  ctx.clearRect(0, 0, canvas.clientWidth, canvas.clientHeight);');
  lines.push('  ctx.setTransform(devicePixelRatio * scale, 0, 0, devicePixelRatio * scale, devicePixelRatio * ox, devicePixelRatio * oy);');
  lines.push('  GRAPH.groups.forEach(function (g) {');
  lines.push('    if (hiddenGroups[g.id]) return;');
  lines.push('    var pts = nodes.filter(function (n) { return n.group === g.id; });');
  ...
  lines.push('      var ex = cx + (p.x - cx) * 1.25, ey = cy + (p.y - cy) * 1.25;');
  lines.push('      if (idx === 0) ctx.moveTo(ex, ey); else ctx.lineTo(ex, ey);');
  lines.push('    });');
  lines.push('    ctx.closePath();');
  lines.push('    ctx.globalAlpha = 0.07; ctx.fillStyle = color; ctx.fill();');
  lines.push('    ctx.globalAlpha = 0.3; ctx.strokeStyle = color; ctx.lineWidth = 1.5 / scale; ctx.stroke();');
  ...
  lines.push('    var inBlast = blast && l.target in blast && l.source in blast;');
  lines.push('    ctx.beginPath();');
  lines.push('    ctx.moveTo(a.x, a.y);');
  lines.push('    ctx.lineTo(b.x, b.y);');
  lines.push('    if (blast && !inBlast) { ctx.globalAlpha = 0.04; }');
  lines.push('    else if (l.linkType === "contains") { ctx.globalAlpha = 0.10; }');
  lines.push('    else if (l.linkType === "decision") { ctx.globalAlpha = 0.5; ctx.setLineDash([4 / scale, 3 / scale]); }');
  lines.push('    else { ctx.globalAlpha = inBlast ? 0.9 : 0.35; }');
  lines.push('    ctx.strokeStyle = inBlast ? "#ff6b6b" : (l.linkType === "decision" ? DECISION_COLOR : "#8b98b8");');
  ...
  lines.push('      ctx.beginPath(); ctx.arc(n.x, n.y, r + 3 / scale, 0, 2 * Math.PI); ctx.stroke();');
  lines.push('    }');
  lines.push('    ctx.globalAlpha = 1;');
  lines.push('  });');
  ...
  lines.push('  nodes.forEach(function (n) {');
  lines.push('    if (hiddenGroups[n.group]) return;');
  lines.push('    var show = n.degree >= maxDeg * 0.3 || (selected && n.id === selected.id) || (blast && n.id in blast && scale > 0.7);');
  lines.push('    if (!show) return;');
  lines.push('    if (blast && !(n.id in blast)) return;');
  lines.push('    ctx.fillText(n.label.length > 34 ? n.label.slice(0, 33) + "..." : n.label, n.x + nodeRadius(n) + 3 / scale, n.y + 4 / scale);');
  lines.push('  });');
  lines.push('}');
  return lines.join(String.fromCharCode(10));
}
...
function scriptInteraction(): string {
  ...
  lines.push('canvas.addEventListener("mousedown", function (e) { dragging = true; moved = false; lastX = e.clientX; lastY = e.clientY; canvas.classList.add("dragging"); });');
  ...
  lines.push('  var wx = (e.offsetX - ox) / scale, wy = (e.offsetY - oy) / scale;');
  ...
  lines.push('document.getElementById("btn-zoom-in").addEventListener("click", function() {');
  lines.push('  var w = canvas.clientWidth, h = canvas.clientHeight;');
  ...
  lines.push('  draw();');
  lines.push('});');
  lines.push('document.getElementById("btn-zoom-out").addEventListener("click", function() {');
  lines.push('  var w = canvas.clientWidth, h = canvas.clientHeight;');
  lines.push('  var wx = (w / 2 - ox) / scale, wy = (h / 2 - oy) / scale;');
  ...
  lines.push('  draw();');
  lines.push('});');
  lines.push('document.getElementById("btn-fit").addEventListener("click", function() { fitView(); draw(); });');
  lines.push('function handleClick(e) {');
  lines.push('  var rect = canvas.getBoundingClientRect();');
  lines.push('  var wx = (e.clientX - rect.left - ox) / scale, wy = (e.clientY - rect.top - oy) / scale;');
  ...
  lines.push('  });');
  lines.push('  if (!best) { selected = null; blast = null; renderInfo(); draw(); return; }');
...
function scriptPanels(): string {
  ...
  lines.push('    muted.textContent = "Click a node to inspect it. Click again to see its blast radius (upstream dependents).";');
  lines.push('    info.appendChild(muted);');
  lines.push('    return;');
  lines.push('  }');
  lines.push('  var n = selected;');
  lines.push('  var title = document.createElement("div");');
  lines.push('  var bold = document.createElement("b"); bold.textContent = n.label;');
  lines.push('  var badge = document.createElement("span"); badge.className = "kind-badge kind-" + n.kind; badge.textContent = n.kind;');
  ...
  lines.push('    b.textContent = "blast radius: " + count + " upstream node" + (count === 1 ? "" : "s");');
  lines.push('    d.appendChild(b);');
  lines.push('    info.appendChild(d);');
  lines.push('  }');
  lines.push('  var neigh = {};');
  lines.push('  (incoming[n.id] || []).forEach(function (l) { neigh[l.source] = "in:" + l.linkType; });');
  lines.push('  (outgoing[n.id] || []).forEach(function (l) { neigh[l.target] = "out:" + l.linkType; });');
  ...
  lines.push('    item.addEventListener("click", function () { selectNode(id); });');
  ...
  lines.push('  nodes.filter(function (n) { return n.label.toLowerCase().indexOf(q) >= 0; })');
  ...
  lines.push('      item.addEventListener("click", function () { selectNode(n.id); });');
  ...
  lines.push('  item.appendChild(dot); item.appendChild(label); item.appendChild(count);');
...
function scriptBoot(): string {
  ...
  lines.push('  GRAPH.stats.nodeCount + " nodes, " + GRAPH.stats.linkCount + " links, " +');
  lines.push('  GRAPH.stats.groupCount + " groups" + (GRAPH.collapsed ? " (collapsed to file level)" : "") +');
  lines.push('  " - generated " + GRAPH.generatedAt.slice(0, 19).replace("T", " ");');
  lines.push('window.addEventListener("resize", resize);');
  lines.push('renderInfo();');
  lines.push('resize();');
  lines.push('function loop() {');
  lines.push('  if (ticksLeft > 0) {');
  lines.push('    for (var i = 0; i < 4 && ticksLeft > 0; i++) { tick(); ticksLeft--; }');
