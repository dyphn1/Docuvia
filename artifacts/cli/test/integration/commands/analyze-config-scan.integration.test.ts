 ...
 */
describe(
  "Command: docuvia analyze (auto mode, empty graph -> full ingestion, real filesystem, full stack)",
  () => {
    ...
    it(
      "[happy] runs the real discovery/config-scan/persist pipeline end-to-end and prints the fused projectType/tags",
      async () => {
        ...
        const summary = lines.find(
          (line) => line.event === "analyze.full.summary",
        );
    ...
    it(
      "[invalid-input] skips a malformed nested package.json without crashing full ingestion",
      async () => {
    ...
    it(
      "[error-handling] exits non-zero with the stable analyze-error boundary when persistence cannot create .docuvia",
      async () => {
        writeFileSync(
          resolve(sandbox.dir, ".docuvia"),
          "not-a-directory",
          "utf8",
        );
        ...
        expect(existsSync(resolve(sandbox.dir, ".docuvia/local.db"))).toBe(
          false,
        );
      },
      35000,
    ...
    it(
      "[state-diff] re-scans changed package metadata on the next full ingestion without duplicating the project row",
      async () => {
    ...
    it(
      "[stress] scans 120 package configs in one real CLI run while deduplicating the fused tag set",
      async () => {
        ...
        const renderedTags = tagsLine
          .slice(
            tagsLine.indexOf("Suggested Tags: ") + "Suggested Tags: ".length,
          )
          .split(",")
          .map((tag) => tag.trim());
