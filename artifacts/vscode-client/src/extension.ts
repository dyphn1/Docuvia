...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("docuvia.clean", async () => {
      const folders = vscode.workspace.workspaceFolders || [];
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("docuvia.status", async () => {
      const folders = vscode.workspace.workspaceFolders || [];
      if (folders.length === 0) return;
      try {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("docuvia.detectChanges", async () => {
      const folders = vscode.workspace.workspaceFolders || [];
      if (folders.length === 0) return;
      try {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("docuvia.sync", async () => {
      const folders = vscode.workspace.workspaceFolders || [];
      if (folders.length === 0) return;
      try {
