import { generateAst, initParser, ParsingFunnel, LanguageRegistry, DefaultProvider } from '@workspace/ast-core';
  ...
  if (type === 'parse') {
    try {
      ...
      if (language && !registry.getProviderForExtension(ext)) {
        registry.registerProvider([ext], new DefaultProvider({
            extensions: [ext], 
      ...
      if (!funnelResult.accepted) {
         (globalThis as any).postMessage({ type: 'error', error: 'File rejected by parsing funnel: ' + funnelResult.reason, filePath });
         return;
      }
