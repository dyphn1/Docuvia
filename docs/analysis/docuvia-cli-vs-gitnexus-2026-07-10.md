...
hermes-agent's test artifacts (`.docuvia/`, `.cursor/`, `.claude/hooks/`, `.windsurfrules`, `.cursorrules`, `llms.txt`, `.github/copilot-instructions.md`, the appended `CLAUDE.md` block, the global `claude_desktop_config.json` MCP entry) have been reverted; hermes-agent and the global Claude config are back to their pre-test state. 6. **Document (and gate behind a flag) `init`'s full side-effect surface**, especially the write to the machine-global Claude Desktop config file.
