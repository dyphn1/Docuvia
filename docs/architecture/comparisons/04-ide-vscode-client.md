...
- **Universal Graph Transparency**: Cursor's context engine is a black box. Docuvia explicitly renders the exact `L3 Intent` and AST Call Graph edges via CodeLens, meaning the developer can visually audit exactly what context the AI is going to see *before* making a prompt.
