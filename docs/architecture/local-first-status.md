...
| :------------------- | :------------------ | :----------------------------------------------------------- | :--------------------------------------------- | :----------- | :-------------------------------------------------------------------- |
| **VS Code Client**   | Editor Integration  | Seamless local IDE experience, Multi-root workspace support. | Work-in-Progress (Integrated with @workspace/core)             | 7            | **code-review-graph-vscode** (MCP tool integration, Webview handling) |
...
| **CLI**              | CLI Tools           | Local `init`, `analyze`, `extract`, `sync`, `query`, and `mcp`. | Core implemented, error handling fixed, uses @workspace/core   | 7            | **GitNexus** (`gitnexus-cli` index, analyze, query)                   |

## 2. Infrastructure & Pipeline

| Component             | Sub-feature         | Local-First Context                                                         | Status / Progress        | Score (1-10) | Internal Workspace Reference (Inspiration)                          |
| :-------------------- | :------------------ | :-------------------------------------------------------------------------- | :----------------------- | :----------- | :------------------------------------------------------------------ |
| **AST Microkernel**   | web-tree-sitter     | Isomorphic offline parsing, native fallback (`native-parsing-fallback`).    | Active (Docs mapped)     | 5            | **code-review-graph** (`parser.py` Tree-sitter + fallbacks)         |
...
| **Local DB**          | SQLite/Drizzle      | High-speed IPC-bypass writing (`local-sqlite-write-pipeline`), L1~L3 nodes. | Active / Schema defined  | 8            | **code-review-graph** (`graph.db` SQLite WAL mode)                  |
| **Incremental Sync**  | Hash Deltas & Hooks | `file-hash-delta-detection`, sub-second updates, Git pre/post-commit.       | Active Development       | 5            | **code-review-graph** (`incremental.py` Git-based change detection) |
...
| Component           | Sub-feature     | Local-First Context                                  | Status / Progress                    | Score (1-10) | Internal Workspace Reference (Inspiration)                          |
| :------------------ | :-------------- | :--------------------------------------------------- | :----------------------------------- | :----------- | :------------------------------------------------------------------ |
| **MCP Server**      | Context Sharing | Standardized local context for Claude/Cursor.        | Active (Integrated with @workspace/core)             | 8            | **code-review-graph** (`main.py` FastMCP server, 30+ tools)         |
| **Intent Router**   | Query Dispatch  | Routing queries to Text, Graph, or Local Embeddings. | Active / Refinement needed           | 6            | **Docuvia** (`intent-router.ts` vector/graph/direct hybrid routing) |
