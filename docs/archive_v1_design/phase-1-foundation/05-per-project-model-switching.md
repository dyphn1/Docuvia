...
## Overview
Allow each project to use a different LLM provider and model, stored and retrieved at runtime.

## Implementation
`lib/db/src/schema/llm_configs.ts` stores per-project config. `artifacts/api-server/src/routes/llm_config.ts` exposes CRUD endpoints.

### Key Files
- `lib/db/src/schema/llm_configs.ts`
- `artifacts/api-server/src/routes/llm_config.ts`
...
- [ ] **Verify 'API route'**: Check that the business rules, data transformations, and edge cases for this entity are fully handled. Ensure the functionality behaves exactly as specified in the requirements.
...
- [ ] **Trigger `Requirement Analyzer` & `Task Verifier`** to perform semantic checks on the logic:
  - **API route**: Trace the implementation from data ingestion/input down to the database or output response. Confirm that all required properties, valid types, and state transitions are explicitly coded.
