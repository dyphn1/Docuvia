# Per-Project Template Inheritance

## Overview

Projects inherit global default templates and can override them at the project level without affecting other projects.

## Implementation

`promptTemplateTemplateType.ts` supports scoping. Template CRUD in `routes/templates.ts`.

### Key Files

- `lib/api-zod/src/generated/types/promptTemplateTemplateType.ts`
...
- [ ] **Verify '(GET/PUT/DELETE per project per type)'**: Check that the business rules, data transformations, and edge cases for this entity are fully handled. Ensure the functionality behaves exactly as specified in the requirements.
- [ ] **Verify 'frontend page'**: Check that the business rules, data transformations, and edge cases for this entity are fully handled. Ensure the functionality behaves exactly as specified in the requirements.
- [ ] **Verify 'editable per-project prompts with default fallback'**: Check that the business rules, data transformations, and edge cases for this entity are fully handled. Ensure the functionality behaves exactly as specified in the requirements.
...
- [ ] **Trigger `Requirement Analyzer` & `Task Verifier`** to perform semantic checks on the logic:
  - **(GET/PUT/DELETE per project per type)**: Trace the implementation from data ingestion/input down to the database or output response. Confirm that all required properties, valid types, and state transitions are explicitly coded.
  - **frontend page**: Trace the implementation from data ingestion/input down to the database or output response. Confirm that all required properties, valid types, and state transitions are explicitly coded.
