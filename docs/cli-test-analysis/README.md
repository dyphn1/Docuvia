...
2. **Docuvia2 Tier B (`--escalate-to-lsp`) has a narrower gate than binary availability.** Fixed `typescript-language-server` resolvability this session (was blocked, same class of issue as C#'s `csharp-ls`), but Tier B's queue is only ever populated by a deterministic (non-LLM) "contract change" classifier in Tier A's delta path, and neither a comment-only edit nor a newly-added exported function triggered it in this session's testing — so a real LSP-precision pass was never exercised, despite the binary now being available. Different blocker shape than C#'s (binary vs. queue-population), still open for a follow-up.
