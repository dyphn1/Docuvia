...
Implements `docuvia analyze <targetPath>` LLM decision extraction (Part A) and follows up on
[analyze.md](analyze.md) 7 claims (Part B), per
`docs/ai_plans/implement_analyze-decision-extraction.md`. Same shape as
[init-concurrency-status.md](init-concurrency-status.md).

## What was verified against `analyze.md`
...
2. Claim 2 and claim 6 original framing in `analyze.md` are stale/overstated (see the table
   ...
   `init-concurrency-status.md` flagged for `init.md`.
