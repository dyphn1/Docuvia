...
日期：2026-08-04（原始分析）；2026-08-05 補充驗證數據  
對照基準：[typescript-cli-benchmark.md](./typescript-cli-benchmark.md)

## 0. 2026-08-05 更新：新驗證數據（本文其餘章節為原始分析，未改動）

本文第 1–9 節寫於 2026-08-04，當時 Docuvia2 的 1,195.6 秒／379,697 edges 這組數字，其實來自一個**尚未修好的 persist 層**：`persistLocked()` 沒有交易包裹，且每個節點插入都會觸發 FTS5 重新 tokenize。2026-08-05 同一天稍晚修好這兩個問題後（新增 `IGraphStore.withTransaction()` 與 `IGraphNodesRepo.withFtsSyncSuspended()`），對同一份 vscode clone 重新完整跑一次，結果是：

```text
Docuvia2（修復後，2026-08-05）：293,309 nodes / 458,614 edges，96m1.450s，PRAGMA integrity_check = 'ok'
CRG（2026-08-04，未重跑）：      231,462 nodes / 1,593,664 edges，約 440 秒
```

也就是說，**修好正確性問題之後，Docuvia2 反而比原本記錄的 1,195.6 秒慢了將近 5 倍**（96 分鐘 vs 20 分鐘），CRG 的相對優勢從原本估計的 2.4–2.7 倍擴大到約 13 倍。這不代表 CRG 變快了，而是代表原本的 1,195.6 秒本身就不是一個「完整且正確」的量測基準——沒有交易包裹的寫入在中途發生 disk I/O 錯誤時，會把 `local.db` 留在不一致的部分寫入狀態（nodes/edges 已寫入但 `project_files` 是空的），這正是本文 §6 當時「懷疑但無法確認」的 persistence 差異假說，這次被直接證實是真實、可測量的影響來源之一——只是量級遠比 §6 猜測的大。

需要誠實記錄的限制：這次修復後的 96 分鐘執行**沒有留下 AST-parse 階段與 persist 階段的分界時間戳**，所以無法把 96 分鐘拆成「純解析」與「純寫入」兩段，也就無法確認 §3（native vs WASM tree-sitter）原本的假說在新數字下還占多少比例——persistence 修復顯然是真實且必要的（避免了資料損毀與磁碟 I/O 錯誤崩潰),但無法宣稱它是唯一或主要的時間增加來源。§7～§9 原本列出的「尚未由實測量化」項目，在這次更新後依然大多數尚未量化，只有「persistence 是否為真實成本來源」這一項從推測變成了確認。

本次也未重新執行 CRG（CRG 數字沿用 2026-08-04 原始記錄，未重測），所以上面的比較仍然只是「新 Docuvia2 vs. 舊 CRG」，不是同一天的公平對照——下一輪驗證若要重新比較兩者速度，CRG 也需要在同一台機器同一時間重跑。

---

## 1. 摘要（原始 2026-08-04 分析，數字已於上方 §0 更新，本節文字保留作為原始記錄）
