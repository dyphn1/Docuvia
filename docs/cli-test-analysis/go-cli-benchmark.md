...
2. **Tier B's behavior on a repo `moby`'s size — CONFIRMED on 2026-08-09 (resolved).** The capability fix plus the receiver-method node_key alignment (defined on [PR #21](https://github.com/dyphn1/Docuvia/pull/21)), verified live: **8,905/10,562 files processed (84.3%), 185,230 cross-file `calls` edges applied**, receiver-method edges (`RouterGroup.GET`-style `file#Recv.Method` keys) confirmed landing in `node_links`. The remaining 1,657 files return `no package metadata` from gopls on macOS (linux/windows build-tags, `_test` files, and `.py`), confirmed by a 7th batch that produced 0 corrected edges — a platform residue, not a graph bug. As of the permanent-failure retry fix (PR #24, from [issue #22](https://github.com/dyphn1/Docuvia/issues/22) / [issue #23](https://github.com/dyphn1/Docuvia/issues/23)), those `no package metadata` files are classified `retryable: false` and dropped from the re-queued `failedEntries` (stamped `markTierBProcessed` instead), so a re-run batch no longer burns time on them — see Session History.
...
- **2026-08-09 (zero-progress watchdog)**: completed [issue #22](https://github.com/dyphn1/Docuvia/issues/22) split item 2 as a cross-batch safety net on top of the permanent-failure fix. Where `retryable: false` handles "the LSP server will never load this file", there remains a class the per-file flag structurally can't reach: a file whose only failure signal is being cut short by the whole-batch deadline reports `retryable: true`, so it stays re-queued forever while producing ~0 edges — the "same files hit their per-file deadline on every batch" case. `run-tier-b-batch` now maintains a meta counter (`tierBZeroProgressBatches`) of **consecutive** zero-progress batches: any real progress (including a partial-timeout batch, and excluding fully-degraded "provider couldn't run at all" runs and batches with no retryable remainder) resets it; the Nth (default 3, mirroring Tier C's poison-pill cap) consecutive zero-progress batch declares the still-retryable `failedEntries` permanently-failed, drops them from the re-queued `remainingQueue`, stamps them `markTierBProcessed`, and reports `zeroProgressWatchdogTripped` (JSONL + CLI). Unit tests cover count-up / no-trip / progress-reset / degraded-untouched / nothing-retryable-untouched / trip-and-stamp; full suite green.
