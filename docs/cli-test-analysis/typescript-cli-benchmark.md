...
**vscode still outstanding:** the same rationale that paused vscode in §3/§4 applies — with sharding now giving nest a ~5-10x unit, vscode is the real stress test of the forward-resolution claim (its `Disposable` hub-symbol reverse amplification, 1,978 project-wide scans per symbol, is exactly what forward removes). A vscode-scale run at `--lsp-processes=N` remains to be scheduled.
