...
| **Orphan Knowledge Branch** | A git orphan branch (e.g., `docuvia-knowledge`) that stores the canonical YAML/Markdown knowledge files independently of the source code tree. The branch has no common ancestor with any source branch. Managed by the Docuvia server via git hook sync. |
...
| **L2 Bootstrap** | The progressive first-run process for a new project: commits are processed in batches of 20, AI self-corrects module names across batches, and the resulting L2 module map is presented for human confirmation. Upon confirmation, module boundaries are fixed as file path patterns in `.docuvia/config.yaml`. |
| **Path Pattern** | A glob pattern (e.g., `src/auth/**`) stored in `.docuvia/config.yaml` under a module definition. After L2 bootstrap confirmation, path patterns provide deterministic (non-LLM) commit-to-module assignment for all future commits. |
...
| **`docuvia sync`** | The CLI command (not yet implemented) triggered by the `post-push` git hook. Uploads the local `.docuvia/` manifest and any newly local-reviewed knowledge to the Docuvia server, which in turn updates the orphan knowledge branch and PostgreSQL. |
