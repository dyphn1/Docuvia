...
- **Daemon Responsibility**: The [AST Microkernel worker](ADR-020-unified-isomorphic-ast-microkernel.md) directly executes `INSERT` and `UPDATE` statements into a [local SQLite database](ADR-002-local-first-architecture.md) (e.g., SQLite or local PostgreSQL/Drizzle schema).
