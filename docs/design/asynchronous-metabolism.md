...
Heavy operations do not execute during the HTTP request cycle. Instead, actions (like capturing a human correction) are written to the database (e.g., `correction_examplesTable`) with a status flag like `status: 'pending'` or `processedAt: null`. The API immediately returns `200 OK`.
