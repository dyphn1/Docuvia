...
6. **Post-Initialization**:
   - Trigger a UI refresh via the [Database-as-IPC](../../adrs/ADR-014-sql-indexed-graph-and-database-as-ipc.md) mechanism, updating the tree view without relying on legacy `FileSystemWatcher` events.
   - Display a success message: `Docuvia: Project "<name>" initialized. Database ready to build your knowledge graph.`
