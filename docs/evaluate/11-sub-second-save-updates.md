...
The Git `post-commit` hook successfully captures knowledge at discrete milestones. However, in modern AI-assisted development (Cursor, Copilot), the AI needs context _before_ the commit happens—often while the file is actively being edited. The local graph must be continuously fresh.
