...
> **Note:** The legacy decision log below (originally arc42 chapter 9, `09-architectural-decisions.md`) uses its own ADR-001–012 numbering scheme that predates the standalone files above and covers a different, overlapping set of early decisions (e.g. its "ADR-002" is about Postgres/JSONB embeddings, not [ADR-002](ADR-002-local-first-architecture.md) above). It is kept here for historical context; treat the 26 standalone files as the current source of truth.

---

# 9. Architectural Decisions (Legacy Log)
...
| [ADR-002](#adr-002-postgresql-with-jsonb-for-embeddings)                        | PostgreSQL (Superseded by [ADR-019: pgvector](./ADR-019-pgvector-migration.md)) | Superseded    |
...
| [ADR-013](./ADR-013-adversarial-implementation-protocol.md)                     | Adversarial Implementation Protocol                                             | Active        |
| [ADR-014](./ADR-014-sql-indexed-graph-and-database-as-ipc.md)                   | SQL-Indexed Graph and Database-as-IPC                                           | Accepted      |
| [ADR-015](./ADR-015-progressive-enrichment-and-ast-lsp-dual-engine.md)          | Progressive Enrichment & AST/LSP Dual Engine                                    | Accepted      |
| [ADR-016](./ADR-016-git-blob-native-identity-and-checkout-thrashing-defense.md) | Git Blob-Native Identity & Checkout Thrashing Defense                           | Accepted      |
| [ADR-017](./ADR-017-tiered-storage-and-orphan-branch-graph-maintenance.md)      | Tiered Storage & Orphan Branch Graph Maintenance                                | Accepted      |
| [ADR-018](./ADR-018-temporal-and-conceptual-bidirectional-linking.md)           | Temporal & Conceptual Bidirectional Linking                                     | Accepted      |
| [ADR-019](./ADR-019-pgvector-migration.md)                                      | PostgreSQL pgvector Migration                                                   | Accepted      |
| [ADR-020](./ADR-020-unified-isomorphic-ast-microkernel.md)                      | Unified Isomorphic AST Microkernel                                              | Accepted      |
...
**Context:**  
[Agentic RAG](./ADR-007-agentic-rag-routing.md) requires semantic search over L2 and L3 node embeddings. External vector databases (Qdrant, Chroma, Pinecone) add infrastructure complexity and an additional deployment dependency. For v1, the node count is expected to stay under ~100K.
...
**Decision:**  
Enforce strict View / Controller / Model separation in both the React frontend and VS Code extension (see [Section 8.3.2](../architecture/crosscutting-concepts.md#832-ui-architecture-mvc) for the full rule specification).
...
The following topics require future architectural decisions. See the [master roadmap](../roadmap/README.md) for implementation plans:

| Topic                                    | Current State               | Reference                                                          |
| ---------------------------------------- | --------------------------- | ------------------------------------------------------------------ |
| Multi-hop graph traversal (BFS/DFS)      | 1-hop only via `node_links` | See [11-risks-and-debt.md R-02](../architecture/risks-and-debt.md) |
| External vector DB migration             | In-memory cosine similarity | See ADR-002 consequences                                           |
| Multi-tenant SaaS architecture           | Single-tenant in v1         | _(Deprecated v1 plan)_                                             |
| Local LLM adapter (Ollama native)        | OpenAI-compatible only      | See ADR-004 consequences                                           |
| VS Code extension distribution (`.vsix`) | No CI packaging step        | See [11-risks-and-debt.md D-02](../architecture/risks-and-debt.md) |
