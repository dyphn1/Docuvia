...
| Direction | One-way **push** only — [publish.ts](../../../../artifacts/cli/src/commands/publish.ts) calls `docuviaApi.sync()`, which POSTs decisions out; there is no fetch/pull | True two-way **sync** — [sync-knowledge.ts:48](../../../../artifacts/cli/src/commands/sync-knowledge.ts#L48) fetches, merges, then pushes |
...
**Scope boundary, deliberately narrow — the CLI-facing presentation layer only.** The rename
touches: `CLI_COMMANDS`/`CLI_COMMAND_DESCRIPTIONS`/`CLI_COMMAND_FLAGS` in `cli-commands.ts`, the
`cli.ts` dispatch entry, `commands/sync.ts` (renamed to `commands/publish.ts`, `syncCommand` →
`publishCommand`), the `SYNC_*` → `PUBLISH_*` keys in `ui-messages.ts`, user-guide (`cli/sync.md` →
`cli/publish.md`, `cli.md`'s command table), `workflows/sync-execution-flow.md` (renamed to
`publish-execution-flow.md`), `adr/cli-driven-architecture.md`'s `docuvia sync` section, and the
associated unit test (renamed to `publish.unit.test.ts`).

It deliberately does **not** touch the orchestration/workflow layer behind `commands/publish.ts`:
`docuviaApi.sync()`, `SyncWorkflow`, `SyncResult`, `SYNC_EVENTS`, the `sync-state.json` lock file,
or the `sync.log` output file all keep their original name. Two reasons: (1) `SyncPushEvent`/
`SyncPushEventTypes` (`@workspace/contracts`) are part of the wire protocol shared with the remote
server — renaming them is an API-contract change, not a CLI-naming cleanup, and is out of scope
here; (2) the rest of that layer (`SyncWorkflow`, the lock/log file names) exists purely to
implement that same wire protocol, so keeping it internally consistent with the protocol's own
vocabulary (not the CLI verb) was judged less confusing than a partial internal rename. A handful
of comments that referenced the CLI command by name (`sync-workflow.ts`, `sync-state.ts`,
`hydrate.ts`, `analyze.ts`) were updated to say `publish` since they describe what a user types,
not the internal class. This also means nothing keyed on the literal string `"sync-knowledge"`
(`PRE_PUSH_SYNC_KNOWLEDGE_MARKER` in `git-constants.ts`, `.husky/pre-push`, `sync-knowledge.ts`)
is affected — that command is unrelated and untouched, as intended.
