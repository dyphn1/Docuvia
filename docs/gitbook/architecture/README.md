...
1. **[Universal VCS Ingestion](../adr/ADR-004-git-isomorphic-graph.md)** — Ingest any Git or SVN repository and produce a queryable knowledge graph from commit history, documents, and build artifacts.
2. **[Three-Tier Knowledge Structure](../adr/README.md#adr-003-three-tier-knowledge-graph-l1l2l3)** — L1 (global classification tags) → L2 (per-project modules/packages) → L3 (implementation decisions, rules, rationale per module).
3. **[Human-in-the-Loop Review](../adr/README.md#adr-006-human-in-the-loop-via-review-queue)** — All AI-generated nodes enter a review queue (`review_tasks`) where humans anchor, merge, or reject them before they are anchored to the graph.
...
| **Extensibility — LLM Providers**   | The LLM integration must be swappable; [OpenAI-compatible interface is an abstraction boundary](../adr/README.md#adr-004-openai-compatible-llm-interface-only)         | 🟠 High     |
| **Maintainability via API-First**   | [All API types are generated from `openapi.yaml`](../adr/README.md#adr-001-openapi-as-single-source-of-truth); zero drift between spec and implementation is mandatory | 🟠 High     |
