...
### [Three-Tier Knowledge Graph](../adr/ADR-005-knowledge-abstraction-strategy.md)
...
**L2 Nodes** — Package, Module, or Component entities scoped to a single project. Extracted from commit diff paths and structure. Linked to L1 Tags. Store an embedding vector ([`pgvector`](../adr/ADR-019-pgvector-migration.md)) enabling semantic search. Stored in `l2_nodes`.

**L3 Nodes** — Implementation Decision, Rule, or Rationale records scoped to an L2 Node. The primary output of the generate pipeline. Store embedding vectors ([`pgvector`](../adr/ADR-019-pgvector-migration.md)). Linked to source commits. Stored in `l3_nodes`.
...
### VCS-based Knowledge Evolver (CQRS & Event Sourcing)

Docuvia adopts a paradigm designed to eliminate the "Cognitive Gap" between human developers and AI Agents. It does this by embedding knowledge directly into the Version Control System using CQRS and Event Sourcing.
...
### [Unified Isomorphic AST Engine & WASM Memory Guardrails](../adr/ADR-020-unified-isomorphic-ast-microkernel.md)
...
2. **[Git Blob Binary Detection](../adr/ADR-016-git-blob-native-identity-and-checkout-thrashing-defense.md)**: Heuristic filtering of binary file types early in the stream.
...
### [Agentic RAG Intent Routing](../adr/ADR-007-agentic-rag-routing.md)
...
### [Human-in-the-Loop Feedback Loop](../adr/ADR-006-self-evolution-architecture.md)
...
- `extension.ts` command handlers, `chat-participant.ts` = **Controller**
- `knowledge-store.ts` = **Model** (YAML ↔ disk)
...
- [02-constraints.md](./constraints.md#23-conventions-coding-rules) — Constraint context for coding rules
- [09-architectural-decisions.md](../adr/README.md#adr-005-mvc-pattern-for-ui-layers) — ADR-005 rationale for MVC decision
- [docs/gitbook/development/vscode-client/00-router-overview.md](../development/vscode-client/00-router-overview.md) — VS Code MVC application in practice
