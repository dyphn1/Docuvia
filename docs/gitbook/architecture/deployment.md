...
All three processes (`api-server`, `kg-engine` dev server, PostgreSQL) can run on the same machine. The [VS Code extension is installed locally](../adr/ADR-002-local-first-architecture.md) in the developer's IDE and connects to `api-server` over HTTP (configurable URL in VS Code settings).
...
| **Static frontend serving**      | Not wired for production                                                         | Vite `dist/` output exists; serving from `api-server` via `express.static()` not yet configured — see [D-03](./risks-and-debt.md) |
| **VS Code extension packaging**  | No `.vsix` build script in CI                                                    | `vsce package` must be run manually; see [D-02](./risks-and-debt.md)                                                              |
