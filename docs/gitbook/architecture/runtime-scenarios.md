...
- **Implementation Route**: `artifacts/api-server/src/routes/ingest.ts` (specifically `POST /projects/:id/ingest/git`)
...
The generate pipeline transforms raw commits into [structured knowledge graph nodes](../adr/ADR-005-knowledge-abstraction-strategy.md). It is protected by an atomic optimistic concurrency lock to prevent parallel execution conflicts.

- **Implementation Route**: `artifacts/api-server/src/routes/generate.ts` (specifically `POST /projects/:id/generate`)
...
## 6.3 Scenario: [Agentic RAG](../adr/ADR-007-agentic-rag-routing.md) Query (MCP)

An AI IDE sends a natural language query via MCP. The [intent router](../adr/ADR-007-agentic-rag-routing.md) classifies it and routes to the best retrieval strategy.

- **Implementation Route**: `artifacts/api-server/src/routes/mcp.ts`
- **Orchestration Logic**: `lib/core/src/services/intent-router.ts` (`routeQuery()`)
...
A reviewer approves an AI-generated L3 decision, creating a [correction example](../adr/ADR-006-self-evolution-architecture.md) for future pipeline runs.

- **Implementation Route**: `artifacts/api-server/src/routes/review-tasks.ts` (specifically `POST /review_tasks/:id/resolve`)
...
See [docs/gitbook/development/vscode-client/command-palette/run-extraction.md](../development/vscode-client/command-palette/run-extraction.md) for the detailed command flow.

- **VS Code Task Runner**: `docs/artifacts/vscode-client/src/task-runner.ts` (`runExtraction()`)
- **Implementation Route**: `artifacts/api-server/src/routes/extensions-vscode.ts` (`POST /extensions/vscode/extract`)
...
- **Implementation Route**: `artifacts/api-server/src/routes/github-webhooks.ts`
- **GitHub Client Wrapper**: `lib/core/src/services/github-client.ts`
...
- **Implementation Route**: `artifacts/cli/src/commands/init.ts` and `artifacts/cli/src/commands/analyze.ts`
...
- **Query Routing**: `lib/core/src/services/query-service.ts` (`getImpact()` with `escalateToLsp` flag)
- **LSP Integration**: `lib/core/src/services/lsp-enrichment-service.ts`
...
- [docs/gitbook/development/vscode-client/command-palette/run-extraction.md](../development/vscode-client/command-palette/run-extraction.md) – Full VS Code extraction flow
- [docs/gitbook/development/vscode-client/chat-participant/slash-commands.md](../development/vscode-client/chat-participant/slash-commands.md) – Chat participant command flows
- [08-crosscutting-concepts.md](./crosscutting-concepts.md#81-domain-model) – Domain model for L1/L2/L3 entities
