...
### 🟩 The Technology Providers (`lib/schema`, `lib/git-local`, `lib/ast-core`, `lib/llm-api`, `lib/remote-api`, `lib/semantic-decision`)

- **Role**: The raw capability wrappers. They interact directly with third-party technologies, file systems, or isolated model runtimes. `lib/semantic-decision` is the PLAT-011 feature-provider boundary: model weights/runtime stay inside that package and only the model-agnostic contract crosses into Docuvia.
...
1. **Tech Providers (`lib/schema`, `lib/git-local`, `lib/ast-core`, `lib/llm-api`, `lib/remote-api`, `lib/semantic-decision`, and the per-language plugin package `lib/plugins-ast`)**: **Strictly Forbidden** for any upper layer (`ui-core`, `artifacts/*`) to import anything from these packages, **including `import type`** (Type A). Tech providers wrap volatile third-party dependencies; allowing type imports would leak those dependencies' shapes (e.g., ORM query objects or Tree-sitter AST nodes) into the orchestration logic, breaking the Virtual Contracts isolation.
...
4. **Implementation-layer directionality (Type B, [PLAT-009](../adr/platform/PLAT-009-ast-core-technology-provider-type-b.md))**: `ast-core` is the raw tree-sitter wrapper (a Technology Provider); `plugins-ast` is its per-language plugin package consuming ast-core's host types. Legal directions, left unlocked: `core → ast-core/plugins-ast` (Domain Core consumes Tech Providers) and `plugins-ast → ast-core` (plugin → host). **PLAT-011 is a deliberate exception:** `core → semantic-decision` is locked because the local model/runtime must be reachable only through `@workspace/contracts` + `TOKENS.SemanticDecisionProvider`. Locked directions also include any Technology Provider importing `lib/core` (upward inversion), any Technology Provider importing a sibling implementation package (cross-import, AGENTS.md mandate 1), and `ast-core → plugins-ast` (host importing its own plugin = cycle). Tech Providers may only import `@workspace/contracts` (plus third-party dependencies) — they never import `lib/core` or each other.
