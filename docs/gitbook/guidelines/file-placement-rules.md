...
- **Rule**: This is the **Domain Core**. It uses the interfaces from `lib/contracts` to perform complex calculations or data transformations. It does not know _how_ the data was fetched from the database, only _what_ to do with it once it has it.
...
Tests must be colocated with the code they are testing, but their _nature_ changes based on the folder:
