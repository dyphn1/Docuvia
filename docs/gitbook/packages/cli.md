...
| `docuvia impact <target>`             | `--escalateToLsp`                   | Blast radius for a symbol: direct/indirect callers and a LOW/MEDIUM/HIGH/CRITICAL risk level, using the same `QueryService.getImpact()` + risk thresholds already used internally by `docuvia review` and the `docuvia_impact` MCP tool. Standalone by-symbol equivalent of asking "what breaks if I change X?" without needing a git diff. |
...
```
cli.ts → impactCommand(target, options)
  → QueryService.getImpact(target, escalateToLsp)  (@workspace/core)
  → SELECT from .docuvia/local.db: node_links (incoming edges only)
  → reuse ChangeDetectionService's IMPACT_RISK_THRESHOLDS for the LOW/MEDIUM/HIGH/CRITICAL rating
  → [optional] LspEnrichmentService — same escalation path as query
```

Exposes the same `getImpact()` logic already used internally by `docuvia review` (per-changed-file blast radius) and by the `docuvia_impact` MCP tool — this command is the missing standalone, by-symbol entry point identified in [`gitnexus-vs-docuvia-full-command-matrix-2026-07-11.md`](../../analysis/gitnexus-vs-docuvia-full-command-matrix-2026-07-11.md).
