...
| ---------------------------------------- | ----------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `docuvia init`                           | —                                   | Initialize the local project: creates `.docuvia/local.db` (SQLite) and the hidden `docuvia-knowledge` branch scaffolding.                                                                                               |
| `docuvia analyze`                        | `--deep`                            | Scan the workspace for file types and project tags. With `--deep`, also triggers background L3 decision extraction.                                                                                                     |
| `docuvia extract <file_path>`            | —                                   | Extract architectural decisions from one specific file via the AST worker pool.                                                                                                                                         |
...
| `docuvia detect-changes`                 | `--baseRef=<ref>`                   | Detect structural changes and a risk score against a git ref (default: working tree diff).                                                                                                                              |
