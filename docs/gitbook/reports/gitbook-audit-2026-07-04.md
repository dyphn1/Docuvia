...
| Multi-provider LLM abstraction (ADR-026)                                      | 📋 Proposed, not Accepted                              | Doc's own status field says "Proposed"; code is still OpenAI-only, contradicting `capabilities-matrix.md`'s 5/5 score                                                                                                                                                                                         |
...
- VS Code Playwright E2E tests — real, 14 test cases across `phase1/2/3.spec.ts`, not stubs as implied.
