...
> Agents MUST use this document alongside [roadmap-checklist.md](roadmap-checklist.md) to verify if the implementation matches the architectural design.

See also: [Roadmap Completion Checklist](roadmap-checklist.md) · [AST Parser Roadmap](./ast-parser-roadmap.md) · [VS Code Extension Roadmap](./vscode-roadmap.md)
...
- **Multi-Provider LLM Abstraction:** Isolate execution and HTTP SSE loops from payload generation using a Thin Transport pattern to support multiple providers (OpenAI, Anthropic, Gemini) (see [ADR-026](../adr/ADR-026-multi-provider-llm-abstraction.md)).
...
- **No In-Memory State:** Avoid storing graph states in Node.js heap. Use [Database-as-IPC](../adr/ADR-014-sql-indexed-graph-and-database-as-ipc.md).
- **Graceful Degradation:** The pipeline must save partial results if the LLM endpoint times out.
- **Vector Math:** Vector search must rely on [pgvector](../adr/ADR-019-pgvector-migration.md), not in-memory cosine similarity (which causes OOMs at scale).
...
- **Cross-Project Soft Linking:** Dynamically query and inject context across linked projects without hard foreign keys, using L1 Global Tags (see [ADR-024](../adr/ADR-024-cross-project-soft-linking.md)).
- **Temporal & Conceptual Linking:** Define explicit 4D graph edge semantics (`IMPLEMENTS`, `EXPLAINS`, `HAS_RULE`) and utilize a Self-Healing Janitor to re-anchor stale rules (see [ADR-018](../adr/ADR-018-temporal-and-conceptual-bidirectional-linking.md)).
...
- **Git Blob Identity:** Files and commits must be anchored by their `git_blob_hash` to prevent checkout thrashing (see [ADR-016](../adr/ADR-016-git-blob-native-identity-and-checkout-thrashing-defense.md)).
...
- **Headless LSP & Hybrid Temp-File Overlay:** Expose LSP-level dirty state tracking to headless CLI/MCP environments via a local file-backed memory index, avoiding RAM limits (see [ADR-025](../adr/ADR-025-hybrid-temp-file-blast-radius.md)).
- **Progressive Enrichment (LSP Dual Engine):** Dynamically boot an LSP to supplement WASM AST analysis for unsaved editor buffers (see [ADR-015](../adr/ADR-015-progressive-enrichment-and-ast-lsp-dual-engine.md)).
...
- **Context Compression:** Replace large code snippets with compressed AST Skeletons before sending to the LLM (see [ADR-010](../adr/ADR-010-context-compression-and-proxy.md)).

### ⚠️ Precautions

- **Memory Leaks:** WASM memory must be manually freed (`tree.delete()`). Failure to do so will crash the Extension Host.
- **Database-as-IPC:** Workers must write parsed graphs directly to SQLite; they must NOT serialize large JSON arrays back to the main thread (see [ADR-014](../adr/ADR-014-sql-indexed-graph-and-database-as-ipc.md) and [ADR-020](../adr/ADR-020-unified-isomorphic-ast-microkernel.md)).
