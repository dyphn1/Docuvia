...
- **Multi-Provider LLM Abstraction:** Isolate execution loops from payload generation using a Thin Transport pattern designed to support multiple providers (OpenAI, Anthropic, Gemini) (see [ADR-026](../adr/ADR-026-multi-provider-llm-abstraction.md)). Currently only OpenAI/Ollama/local adapters are implemented — see [LLM abstraction layer](features/llm-abstraction-layer.md) (✅ Done) for the actual provider coverage.
...
| LLM abstraction layer           | ✅ Done | [View Details](features/llm-abstraction-layer.md)          |
