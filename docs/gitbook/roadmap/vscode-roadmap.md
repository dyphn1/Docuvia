...
- **Initialization:** The `/init` flow creates the SQLite DB ([Database-as-IPC](../adr/ADR-014-sql-indexed-graph-and-database-as-ipc.md)) and spins up the AST Worker ([WASM Microkernel](../adr/ADR-020-unified-isomorphic-ast-microkernel.md)).
- **Outbox Queue:** `docuvia.addDecision` writes payloads directly to the local DB and the `SyncOutbox`.

### ⚠️ Precautions

- **No Direct YAML Writes:** Data must be written to SQLite, avoiding the deprecated `.docuvia` YAML configs ([ADR-002](../adr/ADR-002-local-first-architecture.md)).
- **Non-blocking UI:** Commands like extraction must not freeze the editor. Heavy lifting is dispatched to the background via [Asynchronous Metabolism](../adr/ADR-008-asynchronous-metabolism.md).
...
- **Legacy Abstraction:** Follow the strict three-tier [Knowledge Abstraction Strategy](../adr/ADR-005-knowledge-abstraction-strategy.md).
...
- **Progressive Enrichment:** Use the AST Microkernel to identify symbols for Hover popups instead of brittle UUID regex matching ([ADR-015](../adr/ADR-015-progressive-enrichment-and-ast-lsp-dual-engine.md)).
...
- **Chat Participant (`@docuvia`):** Register slash commands (`/explore`, `/query`, `/extract`). Connect `/query` to the local [Agentic RAG Router](../adr/ADR-007-agentic-rag-routing.md).
...
- **Path Traversal Security:** Since files are no longer read from disk (replaced by [Database-as-IPC](../adr/ADR-014-sql-indexed-graph-and-database-as-ipc.md)), raw `filePath` payloads are deprecated in favor of `nodeId` lookups.
