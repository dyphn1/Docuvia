...
- `--tier-c-all`: Only relevant with `--escalate-to-lsp`. Drains every queued Tier C item in one run, removing the wall-clock and item-count caps. Budget exhaustion and per-item poison-pill eviction (3 consecutive failures → removal) still apply. Useful for catching up when the Tier C queue is large; without this flag, each `analyze` run processes at most 20 items within a 12-second window.
