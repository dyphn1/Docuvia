...
- `--skip-git`: Skip the Git remote reachability check and the git-hook health check (both need `IGitProvider`). The Tier B commit-cap check is gated by `--skip-db` instead — it reads a store-persisted counter, not git. The post-commit ingestion check (health check 9 below) needs both the db and git, so it is skipped by either flag.
...
8. **LSP Binary Presence (`lsp_binary_<language>`)**: Reports whether each queued language's LSP resolves, independent of whether a Tier B batch has ever run — reuses the exact same pre-flight gate `analyze --escalate-to-lsp` itself uses, scoped to only the language(s) actually present in the Tier B queue (so an unrelated language's missing binary never fails doctor for a project that doesn't use it). `FAIL` when unavailable — an unready LSP environment wastes a full `analyze --escalate-to-lsp` batch's wall-clock time and produces misleadingly incomplete results, so this is a real defect, not merely informative.
9. **Post-Commit Ingestion Health (`post_commit_ingestion`)**: Detects a dead post-commit hook pipeline that the hook-file check above structurally can't — a hook file can be perfectly installed and still never run, because its backgrounded `&` process can die with the hook's shell (or `npx --no-install` can fail before any JSONL is written), silently leaving `lastIngestedSourceSha` behind `HEAD` forever. Compares `lastIngestedSourceSha` against `HEAD`: `PASS` when they match (graph fully up to date); `PASS` with a note when behind `HEAD` but an `analyze` run completed within the recent grace window (likely still in flight from the post-commit hook); `FAIL` when behind `HEAD` with no recent `analyze.log` activity — the exact symptom of issue #58 — suggesting a manual `docuvia analyze` and a look at `.docuvia/logs/post-commit-hook.log`. Every message also carries the current Tier C queue size, so a permanently-empty queue is visible rather than silent. Silently skipped when no db / unborn `HEAD` (already covered by other checks).
