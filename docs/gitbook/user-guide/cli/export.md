...
- `--topology` *(Required)*: Generates an interactive HTML/JS topology map of the codebase.
...
1. **Read-Side Query**: The system queries the `IGraphNodesRepo` to fetch `getAllNodes` and `getAllLinks`. 
   > *Warning: In massive codebases, this loads the entire graph into memory, which carries an Out-Of-Memory (OOM) risk.*
