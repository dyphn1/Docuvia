...
The remaining forms (a plugin path built from a runtime variable, `import()` with a computed specifier, `child_process` spawning another project file) still cannot be resolved into exact edges — but since issue #217 they are no longer invisible: the raw call sites survive in `ast_call_sites`, and the blast-radius fallback above surfaces the calling files as `edgeSource: "lsp-fallback"` entries. An empty result now means "no static edge _and_ no same-named call site anywhere", which is a much stronger zero — though it still carries the issue #192 `UNKNOWN`/lower-bound caveats rather than presenting itself as a confident answer.
