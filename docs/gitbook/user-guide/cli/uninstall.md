...
- `--platform=<slug1,slug2,...>`: Only remove the named integrations. Available slugs: `cursor`, `claude`, `copilot`, `codex`, `continue`, `hermes`. Omit to remove every integration installed by `init` — the same interactive checkbox / all-platforms default described in [`docuvia init`](init.md) applies here too.
