...
> directly and `docuviaApi.clean()` for the database half.

`docuvia uninstall` reverses what `init` installed: per-platform hooks/rules/MCP config, and
(unless `--keep-db`) the local database. It has no dedicated Orchestration-layer workflow of its
own for the hooks half — it calls each platform's `uninstallHooks()` directly.
