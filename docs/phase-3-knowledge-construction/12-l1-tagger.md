# L1 Tagger — Universal Taxonomy

## Overview
AI-generate candidate L1 classification tags into a shared global pool. New projects inherit and extend the pool.

## Implementation
`lib/db/src/schema/l1_tags.ts`, `artifacts/api-server/src/routes/l1_tags.ts`. Full CRUD via `l1Tag.ts`, `l1TagInput.ts`, `l1TagUpdate.ts`.

### Key Files
- `lib/db/src/schema/l1_tags.ts`
- `artifacts/api-server/src/routes/l1_tags.ts`
- `lib/api-zod/src/generated/types/l1Tag.ts`
