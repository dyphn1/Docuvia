...
The LLM abstraction layer is **OpenAI-only** at runtime. While the database schema (`llm_configs` table) includes a `provider` column that suggests multi-provider support, no code consumes this column to switch between LLM providers. All routes import a singleton `openai` client (OpenAI SDK) pointed at `AI_INTEGRATIONS_OPENAI_BASE_URL`. Setting `provider: "anthropic"` in the database has zero runtime effect — all requests still go through the OpenAI SDK.

Additionally, three separate `new OpenAI(...)` client instances are created (`client.ts`, `image/client.ts`, `audio/client.ts`) using identical configuration, creating redundancy without functional benefit.

### Recommended Fix

1. **Option A (minimal)**: Remove or deprecate the `provider` column from `llm_configs` to avoid user confusion. Document that the system supports any OpenAI-compatible API (LiteLLM, OpenAI API, etc.) via `baseURL` configuration, not provider abstraction.

2. **Option B (full multi-provider)**: Implement a client factory that reads `provider` from `llm_configs` and instantiates the appropriate SDK (Anthropic SDK, Google Generative AI, etc.). This requires:
   - A provider-to-client mapping module
   - Route changes to use the factory instead of the singleton
...
1. **Per-project model switching**: `generate.ts` reads `model` from `llm_configs` table per project (line 76-83) and passes it to `openai.chat.completions.create()`. The `PATCH /projects/:id/llm-config` route allows runtime model updates via authenticated API.
...
1. **⚠️ `provider` field is decorative**: The `llm_configs.provider` column (default `"openai"`) exists in the schema and is configurable via API, but no runtime code reads it to select a different LLM client. Setting any provider value other than `"openai"` has zero effect.

2. **⚠️ Three redundant client instances**: `client.ts`, `image/client.ts`, and `audio/client.ts` each instantiate `new OpenAI({...})` with identical env vars. This is wasteful and could cause config drift.

3. **⚠️ No provider abstraction layer**: There is no interface/abstract class for LLM clients. All code is tightly coupled to the OpenAI SDK's `chat.completions.create`, `embeddings.create`, `images.generate`, etc.
...
1. **Env-var-based secrets**: API keys read from environment (not hardcoded). `client.ts` requires both `AI_INTEGRATIONS_OPENAI_BASE_URL` and `AI_INTEGRATIONS_OPENAI_API_KEY` (or `OPENAI_API_KEY` fallback).

2. **Auth on config routes**: `llm_config.ts` uses `requireApiKey` middleware on both GET and PATCH routes.

3. **Zod input validation**: `LlmConfigInputSchema` validates PATCH body (`model: z.string().optional()`, `provider: z.string().optional()`).

4. **Error handling**: `generate.ts` wraps LLM calls in try/catch with structured logging. `embedding.ts` returns `null` on failure (graceful fallback to SQL LIKE search).

5. **Resource cleanup**: `audio/client.ts` uses `finally` block to clean up temp files from ffmpeg conversion.

### Issues Found

1. **⚠️ Schema-code mismatch**: `llm_configs.provider` column exists but is unused. This creates a false API contract — users can set `provider: "anthropic"` but the system ignores it.

2. **⚠️ Redundant client singletons**: Three modules each create their own OpenAI instance. If env vars change at runtime (e.g., via `process.env` reassignment), instances could diverge.

3. **🟡 Fallback API key**: `client.ts` falls back from `AI_INTEGRATIONS_OPENAI_API_KEY` to `OPENAI_API_KEY`. This is intentional for proxy compatibility but could mask misconfigurations.
...
1. **Singleton usage**: `openai` from `client.ts` is imported by 6 route/module files (`generate.ts`, `intent-router.ts`, `embedding.ts`, `metabolism.ts`, `pull_requests.ts`, `github_webhooks.ts`). All share the same configuration. ✅

2. **Per-project model routing**: `generate.ts` correctly reads `llm_configs` per project and passes the model name to API calls. ✅

3. **Batch processing**: `batch/utils.ts` is a generic utility (not imported by any route currently) — available for future batch operations but not actively used in the knowledge generation pipeline.

### Missing Coverage

1. **Non-OpenAI providers**: Despite the `provider` schema field, only OpenAI SDK is wired. Anthropic, Google, Cohere, and local models (Ollama) are not supported.

2. **Provider-specific features**: Image generation uses `gpt-image-1` (OpenAI-specific model name). Audio uses `gpt-audio` and `gpt-4o-mini-transcribe` (OpenAI-specific). These would need provider-specific abstractions for multi-provider support.

3. **Batch utilities unused**: `batchProcess` and `batchProcessWithSSE` are exported but not imported anywhere in `artifacts/api-server/src/`. The knowledge generation pipeline implements its own batching in `generate.ts`.
