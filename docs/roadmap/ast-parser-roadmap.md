...
- [x] **關聯建立 (Edges)**：將萃取出的 `call` 與 `import` 轉化為 `node_links` 資料表中的 `CALLS` 或 `DEPENDS_ON` 邊 (Edges)。

**Phase 3 完成狀態**：✅ 2026-06-25 — `ast-ingestion-pipeline.ts` 已實作 File→L2、Class/Function→L3 映射及 call/import→node_links 關聯建立。
