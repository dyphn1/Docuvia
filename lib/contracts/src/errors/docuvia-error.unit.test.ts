...
describe("DocuviaError", () => {
  ...
  describe(".wrap()", () => {
    it("wraps a native Error, prefixing the message and preserving it as cause", () => {
      ...
      const wrapped = DocuviaError.wrap(
      ...
      expect(wrapped.message).toBe(
        "Failed to read file: ENOENT: file not found",
      );
    ...
    it("passes an already-DocuviaError straight through unchanged (no double-wrapping)", () => {
      const original = new DocuviaError(
        ErrorCodes.GIT_NOT_A_REPOSITORY,
        "not a repo",
      );
      const wrapped = DocuviaError.wrap(
        ErrorCodes.GIT_COMMAND_FAILED,
