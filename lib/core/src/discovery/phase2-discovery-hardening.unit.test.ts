...
describe("Phase 2 discovery contract hardening", () => {
  ...
  it("VCS hotspot discovery returns no tags outside a git repository without reading history", async () => {
    const getRecentChangedFilePaths = vi.fn().mockResolvedValue(["src/auth/a.ts"]);
    const git = gitProvider({
      isGitRepository: vi.fn().mockResolvedValue(false),
      getRecentChangedFilePaths,
    });

    await expect(new VcsScannerService(git).extractHotspotTags(tmpDir)).resolves.toEqual([]);
  ...
  it("VCS hotspot discovery degrades to an empty result when git history access fails", async () => {
    const git = gitProvider({
      isGitRepository: vi.fn().mockResolvedValue(true),
      getRecentChangedFilePaths: vi.fn().mockRejectedValue(new Error("git unavailable")),
    });
  ...
  it("file discovery falls back to filesystem acquisition when git candidate enumeration fails", async () => {
    fs.writeFileSync(path.join(tmpDir, "fallback.ts"), "export const fallback = 1;\n");
    const git = gitProvider({
      isGitRepository: vi.fn().mockResolvedValue(true),
      listTrackedFilesWithBlobHash: vi.fn().mockRejectedValue(new Error("broken index")),
    });

    const result = await new FileDiscoveryService(git).discoverFiles(tmpDir, emptyHashes);
  ...
  it("file discovery only-indexed mode excludes dirty and untracked paths from the output", async () => {
    const git = gitProvider({
      isGitRepository: vi.fn().mockResolvedValue(true),
      listTrackedFilesWithBlobHash: vi.fn().mockResolvedValue(new Map([["tracked.ts", "blob-sha"]])),
  ...
  it("file discovery is deterministic across repeated identical filesystem input", async () => {
    ...
    const service = new FileDiscoveryService(gitProvider());
  ...
  it("config discovery safely falls back to generic/general for a missing workspace", async () => {
    ...
    await expect(new ConfigScannerService().scanConfigs(missing)).resolves.toEqual({
      projectType: "generic",
      tags: ["general"],
    });
  });
