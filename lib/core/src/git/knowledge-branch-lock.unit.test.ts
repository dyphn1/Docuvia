...
describe("withKnowledgeBranchLock()", () => {
  it("acquires before running fn and releases after it resolves", async () => {
    ...
    const order: string[] = [];
    (git.acquireKnowledgeLock as ReturnType<typeof vi.fn>).mockImplementation(async () => {
      order.push("acquire");
    });
    (git.releaseKnowledgeLock as ReturnType<typeof vi.fn>).mockImplementation(async () => {
      order.push("release");
    });

    const result = await withKnowledgeBranchLock(git, "/workspace", async () => {
      order.push("fn");
      return "done";
    });
  ...
  it("does not run fn or release when acquiring the lock itself throws", async () => {
    const git = makeMockGitProvider({
      acquireKnowledgeLock: vi.fn().mockRejectedValue(new Error("lock timeout")),
    });
    const fn = vi.fn();

    await expect(withKnowledgeBranchLock(git, "/workspace", fn)).rejects.toThrow("lock timeout");
