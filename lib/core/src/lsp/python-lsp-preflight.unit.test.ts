...
describe("checkPythonLspPreflight()", () => {
  ...
  it(
    "reports not ready with a reason when no pyproject.toml/requirements.txt is present",
    async () => {
  ...
  it(
    "accepts requirements.txt as an alternative marker to pyproject.toml",
    async () => {
  ...
  it(
    "reports the LSP binary as unresolvable when neither a local copy nor npx can find pyright",
    async () => {
      fs.writeFileSync(
        path.join(workspaceRoot, "pyproject.toml"),
        "[project]\n",
  ...
  it(
    "resolves locally when node_modules/.bin has a pyright-langserver copy",
    async () => {
      fs.writeFileSync(
        path.join(workspaceRoot, "pyproject.toml"),
        "[project]\n",
