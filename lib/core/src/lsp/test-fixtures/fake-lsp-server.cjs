...
function handleMessage(body) {
  ...
  if (msg.method === "getEnv") {
    // Round-trips the spawned process's own env so tests can assert exactly
    // what the client passed to spawn() (issue #165: default-env allowlisting).
    send({ jsonrpc: "2.0", id: msg.id, result: process.env });
    return;
  }
  if (msg.method === "hang") {
  ...
  if (msg.method === "fail") {
    send({
      jsonrpc: "2.0",
      id: msg.id,
      error: { code: -1, message: "fake failure" },
    });
    return;
  }
