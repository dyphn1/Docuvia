...
describe("buildWindowsShellCommandLine()", () => {
  // The `where` probe this exercises is Windows-only (`where` doesn't exist on macOS/Linux), and
  // the module's `promisify`ed `execFile` can't be mocked from a test (it closes over the original
  // Node builtin at import time), so this genuinely can only run where `where` is real.
