...
/**
 * `graph` repo — `l2_nodes` / `node_links` persistence, redistributed from
 * old Docuvia's `SqliteGraphRepository.persistAstGraph`. Path-pattern
 * matching mirrors the old repo's convention: a node's `path_patterns`
 * column stores a single-element JSON array `["<file path>"]`.
 */
export class GraphNodesRepo {
  constructor(private readonly db: Database.Database) {}

  /**
   * Deletes any existing l2_nodes for `filePath` (and their outgoing
   * node_links / l2_node_l1_tags rows), so a re-parsed file's stale nodes
   * don't linger. Returns the deleted node ids.
   */
