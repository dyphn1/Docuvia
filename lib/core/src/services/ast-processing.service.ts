...
export class AstProcessingService implements IAstProcessor {
  ...
  // Old Docuvia let this default internally, which is exactly the DI-bypass pattern this
  // rebuild's "no constructor defaults" rule exists to prevent (see the composition-root
  // convention: only `build*Capability()` functions are allowed to `new` a concrete service).
