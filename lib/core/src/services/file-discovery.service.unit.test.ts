...
/**
 * Mocks the narrower `ProjectFilesRepo` dependency `FileDiscoveryService` now takes instead of
 * a raw `dbPath` (which it used to open its own `better-sqlite3` connection against). Defaults
 * to "no existing hashes" — equivalent to old Docuvia's "dbPath does not exist" fixture case.
 */
function makeMockFilesRepo(
  ...
  return {
    ...
    upsertFile: vi.fn(),
  } as unknown as ProjectFilesRepo;
}
...
describe("FileDiscoveryService", () => {
  ...
  it("does not re-parse a file whose hash matches the ProjectFilesRepo's existing hash", async () => {
