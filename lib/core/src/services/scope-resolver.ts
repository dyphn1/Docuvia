...
/**
 * Ported as-is from old Docuvia's `services/scope-resolver.ts`. Not called out explicitly in
 * the migration plan's "port first" list, but required for behavioral parity of
 * `run-parse-and-persist.ts`'s cross-file call/implements/extends edge resolution — the
 * logic old `SqliteGraphRepository.persistAstGraph()` used, now redistributed into
 * `run-parse-and-persist.ts` against `GraphStore.graph`'s primitives (see that file's
 * `persistAstGraph` helper, whose `findNodeIdByName` fallback chain mirrors this file's
 * original caller 1:1).
 */
