 ...
 *
 * Allows multiple concurrent readers while granting writers exclusive access.
 * Waiters are served in FIFO order; once a writer is queued, later readers wait
 * behind it so writers cannot be starved. Used to serialize SQLite writes so
 * parallel swarm agents never crash with SQLITE_BUSY ("database is locked").
 */
