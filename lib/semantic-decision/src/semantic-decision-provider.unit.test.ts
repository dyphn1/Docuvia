...
describe("SemanticDecisionProvider foundation boundary", () => {
  ...
  it("[invalid-input] never invents a score when the bounded option set is empty", async () => {
  ...
  it("[error-handling] degrades honestly instead of throwing while the package-owned model is unavailable", async () => {
    const provider = new SemanticDecisionProvider();

    await expect(provider.checkAvailability()).resolves.toEqual({
      available: false,
      reason: "semantic decision model is not installed in this foundation slice",
    });
