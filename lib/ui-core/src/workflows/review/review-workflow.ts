...
export class ReviewWorkflow {
  ...
  public async execute(baseRef?: string): Promise<ReviewResult> {
    ...
    await appendReviewLogLine(workspaceRoot, { event: "review.start", baseRef: baseRef ?? null });
    ...
    try {
      ...
      if (err instanceof DocuviaError && err.code === ErrorCodes.DB_OPEN_FAILED) {
        ...
        throw new DocuviaError(ErrorCodes.DB_OPEN_FAILED, REVIEW_MESSAGES.DB_NOT_FOUND, err);
      }
      throw err;
    }

    try {
      const changeDetectionService = docuviaFactory.resolve(TOKENS.ChangeDetectionService, { logger });
